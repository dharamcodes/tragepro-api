...
import com.tragepro.api.identity.dto.RoleType;
...
class AuthenticationControllerTest extends ApiTestSetup {
  ...
  void setUp() {
    authenticationRequest =
        AuthenticationRequest.builder()
            ...
            .build();
    loginRequest = LoginRequest.builder().userName(uuid).password("TestPassword123").build();
  }

  @Test
  void testSignup_Success() throws Exception {
    AuthenticationRequest newSignup =
        AuthenticationRequest.builder()
  ...
  void testGetByUserName_Success() throws Exception {
    mockMvc
        .perform(get("/api/v1/auth/find/{userName}", uuid).header("Authorization", authToken))
        .andExpect(status().isOk())
  ...
  void testGetByUserName_NotFound() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/auth/find/{userName}", "nonexistentUser")
  ...
  void testUpdateAuthenticationDetails_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/auth/update/{userName}", uuid)
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void testUpdateAuthenticationDetails_Exception() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/auth/update/{userName}", "nonexistentUser")
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updatedRequest)))
        .andExpect(status().isNotFound());
  ...
  void testLogin_Exception() throws Exception {
    LoginRequest nonexistentRequest =
        LoginRequest.builder().userName("nonexistentUser").password("somePassword123").build();
    mockMvc
        .perform(
            post("/api/v1/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void testResetPassword_Success() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/auth/reset-password/{userName}", loginRequest.userName())
                .header("Authorization", authToken))
  ...
  void testResetPassword_Exception() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/auth/reset-password/{userName}", UUID.randomUUID().toString())
                .header("Authorization", authToken))
        .andExpect(status().isNotFound());
  }
  ...
  void testChangePassword_Exception() throws Exception {
    ...
    ResetPasswordRequest resetPasswordRequestInactive =
        ResetPasswordRequest.builder()
            .userName(UUID.randomUUID().toString())
            .password("NewPassword123!")
            .confirmPassword("NewPassword123!")
  ...
  void testDeactivateUser_Success() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/auth/deactivate/{userName}", authenticationRequest.userName())
                .header("Authorization", authToken))
        .andExpect(status().isAccepted());
    mockMvc
        .perform(
  ...
  void testDeactivateUser_Exception() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/auth/deactivate/{userName}", "userName")
                .header("Authorization", authToken))
        .andExpect(status().isNotFound());
  }
  ...
  void testDeleteUser_Success() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/auth/delete/{userName}", authenticationRequest.userName())
                .header("Authorization", authToken))
        .andExpect(status().isAccepted());
  }
  ...
  void testDeleteUser_Exception() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/auth/delete/{userName}", "nonexistentUser")
                .header("Authorization", authToken))
        .andExpect(status().isNotFound());
  }

  @Test
  void testAuthFilter_InvalidToken_Returns401() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/auth/find/{userName}", uuid)
                .header("Authorization", "Bearer invalidTokenString123"))
        .andExpect(status().isUnauthorized());
  }

  @Test
  void testAuthFilter_DifferentRoles() throws Exception {
    for (RoleType role :
        new RoleType[] {
          RoleType.APP_ADMIN, RoleType.APP_MANAGER, RoleType.SUPER_USER, RoleType.APP_USER
        }) {
      String customUser = UUID.randomUUID().toString();
      ...
      String token =
          "Bearer "
              + jwtTokenHelper.generateToken(
                  customUser, Map.of("role", role.getValue(), "user", customUser));

      mockMvc
          .perform(get("/api/v1/auth/find/{userName}", customUser).header("Authorization", token))
          .andExpect(status().isOk());
    }
  }
}
