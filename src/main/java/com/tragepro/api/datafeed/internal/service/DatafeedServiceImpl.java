...
public class DatafeedServiceImpl implements DatafeedService {
  ...
  public LoadCandleResponse loadData(LoadCandleRequest request) {
    ...
    // Find the watchlist by name using Java streams
    var watchlist =
    ...
    stocks.forEach(
        stock -> {
          try {
            ...
            // Resolve securityId from securityService
            ...
            try {
              ...
              return; // returns from lambda, continuing to next stock in forEach
            }
            ...
            // Update state in context to COMPLETED
              ...
              executor.submit(
                  () -> {
                    ...
                    if (!candleService.isCandleExists(enrichedSymbol.name(), timestamp)) {
                      var enriched =
                          com.tragepro.api.datafeed.dto.CandleRequest.builder()
