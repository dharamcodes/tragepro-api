...
public class TimeframesUtil {
    ...
    private static CandleDataModel aggregate(Map.Entry<Long, List<CandleDataModel>> entry) {
        ...
        return new CandleDataModel(
                ...
                first.open(),
                candles.stream()
                        .map(CandleDataModel::high)
                        .max(Double::compareTo)
                        .orElse(0.0),
                candles.stream()
                        .map(CandleDataModel::low)
    ...
    private static long bucketStart(long timeStamp, TimeframeModel tf) {
        ...
        return switch (tf.getUom()) {
            ...
            case WEEK ->
                time.with(DayOfWeek.MONDAY)
                        .truncatedTo(ChronoUnit.DAYS)
                        .toInstant()
                        .toEpochMilli();
            case MONTH -> {
                int bucketMonth = ((time.getMonthValue() - 1) / tf.getValue()) * tf.getValue() + 1;
                yield ZonedDateTime.of(LocalDate.of(time.getYear(), bucketMonth, 1), LocalTime.MIN, time.getZone())
                        .toInstant()
                        .toEpochMilli();
            }
        };
