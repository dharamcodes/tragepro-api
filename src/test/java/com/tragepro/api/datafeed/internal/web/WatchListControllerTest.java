...
import com.tragepro.api.datafeed.internal.entity.WatchListEntity;
...
class WatchListControllerTest extends ApiTestSetup {
  ...
  void testGetById_Success() throws Exception {
    mockMvc
        .perform(get("/api/v1/watchlists/{id}", savedId).header("Authorization", authToken))
        .andExpect(status().isOk())
  ...
  void testGetById_NotFound() throws Exception {
    mockMvc
        .perform(get("/api/v1/watchlists/{id}", "nonExistentId").header("Authorization", authToken))
  ...
  void testGetAll_Success() throws Exception {
    mockMvc
        .perform(get("/api/v1/watchlists").header("Authorization", authToken))
        .andExpect(status().isOk())
  ...
  void testUpdate_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/watchlists/{id}", savedId)
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void testPatch_Rename_Success() throws Exception {
    WatchListRequest patchRequest = WatchListRequest.builder().name("New List Name").build();

    mockMvc
        .perform(
            patch("/api/v1/watchlists/{id}", savedId)
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void testPatch_AddStock_Success() throws Exception {
    ...
    stocks.add(newStock);
    WatchListRequest patchRequest = WatchListRequest.builder().stocks(stocks).build();
  ...
  void testDelete_Success() throws Exception {
    mockMvc
        .perform(delete("/api/v1/watchlists/{id}", savedId).header("Authorization", authToken))
        .andExpect(status().isOk());
  ...
  void testUpdate_NotFound() throws Exception {
    WatchListRequest updateRequest = WatchListRequest.builder().name("Non-existent").build();
    mockMvc
        .perform(
            put("/api/v1/watchlists/{id}", "nonExistentId")
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateRequest)))
        .andExpect(status().isNotFound());
  ...
  void testDelete_NotFound() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/watchlists/{id}", "nonExistentId").header("Authorization", authToken))
        .andExpect(status().isNotFound());
  }

  @Test
  void testPatch_NotFound() throws Exception {
    WatchListRequest patchRequest = WatchListRequest.builder().name("Non-existent").build();
    mockMvc
        .perform(
            patch("/api/v1/watchlists/{id}", "nonExistentId")
                .header("Authorization", authToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(patchRequest)))
        .andExpect(status().isNotFound());
