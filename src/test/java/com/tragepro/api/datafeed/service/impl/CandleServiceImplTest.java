...
import com.tragepro.api.datafeed.core.repository.CandleRepository;
...
class CandleServiceImplTest {
    ...
    void isCandleExists_True() {
        ...
        assertTrue(exists);
        verify(candleRepository, times(1)).existsBySymbolDataNameAndCandleDataTimestamp("Apple Inc.", 12345L);
    }
    ...
    @Test
    void create_Success() {
        when(mapperFactory.getMapper(CandleMapper.class)).thenReturn(candleMapper);
    ...
    void getAll_Success() {
        when(candleRepository.findAll(any(org.springframework.data.domain.Pageable.class)))
                .thenReturn(new org.springframework.data.domain.PageImpl<>(java.util.List.of(candleEntity)));
        when(mapperFactory.getMapper(CandleMapper.class)).thenReturn(candleMapper);
        when(candleMapper.entityToResponse(any())).thenReturn(candleResponse);
    ...
    void getAll_Empty() {
        ...
        assertThrows(
                AppException.class, () -> candleService.getAll(org.springframework.data.domain.Pageable.unpaged()));
    }

    @Test
    void getAllSet_Success() {
        when(candleRepository.findAll()).thenReturn(java.util.List.of(candleEntity));
        when(mapperFactory.getMapper(CandleMapper.class)).thenReturn(candleMapper);
        when(candleMapper.entityToResponse(any())).thenReturn(candleResponse);

        java.util.Set<CandleResponse> response = candleService.getAll();
    ...
    @Test
    void getAllSet_Empty() {
        when(candleRepository.findAll()).thenReturn(java.util.Collections.emptyList());

        assertThrows(AppException.class, () -> candleService.getAll());
    }
    ...
    void getCandlesBySymbolAndDaysBack_Success() {
        when(candleRepository.findBySymbolDataNameAndCandleDataTimestampGreaterThanEqual(anyString(), anyLong()))
                .thenReturn(java.util.List.of(candleEntity));
        when(mapperFactory.getMapper(CandleMapper.class)).thenReturn(candleMapper);
        when(candleMapper.entityToResponse(any())).thenReturn(candleResponse);

        java.util.List<CandleResponse> response = candleService.getCandlesBySymbolAndDaysBack("AAPL", 5);
    ...
    void getLatestCandlesBySymbols_Success() {
        ...
        when(candleRepository.findLatestCandlesBySymbols(any())).thenReturn(java.util.List.of(candleEntity));
        when(mapperFactory.getMapper(CandleMapper.class)).thenReturn(candleMapper);
        when(candleMapper.entityToResponse(any())).thenReturn(candleResponse);
