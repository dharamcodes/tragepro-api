...
class StrategyMapperTest {
    ...
    @Test
    void testGetMapperClass() {
        assertEquals(StrategyMapper.class, mapper.getMapperClass());
    }
    ...
    void testRequestToEntity_FullyPopulated() {
        StrategyModel strategy =
                StrategyModel.builder().name("Test Strategy").desc("Desc").build();
        SymbolModel symbolData = SymbolModel.builder()
                .symbol("AAPL")
                .name("Apple")
    ...
    void testEntityToResponse_FullyPopulated() {
        StrategyModel strategy =
                StrategyModel.builder().name("Test").desc("Desc").build();
        SymbolModel symbolData = SymbolModel.builder()
                .symbol("AAPL")
                .name("Apple")
                .exchange(Exchange.NSE)
    ...
    void testMerge() {
        StrategyModel strategy =
                StrategyModel.builder().name("Updated Name").desc("Updated").build();
        SymbolModel symbolData = SymbolModel.builder()
        ...
        target.setSymbolData(SymbolModel.builder()
                .symbol("Old Symbol")
                .name("Old")
                .exchange(Exchange.NSE)
                .build());
    ...
    void testToSymbolData() {
        SymbolModel symbolModel =
                SymbolModel.builder().symbol("AAPL").name("Apple").build();
    ...
    void testResponseToRequest() {
        ...
        StrategyResponse response = StrategyResponse.builder()
                .strategy(StrategyModel.builder().name("Test").build())
                .build();
    ...
    void testPartialNulls() {
        StrategyRequest req1 =
                StrategyRequest.builder().strategy(null).symbolData(null).build();
        StrategyEntity ent1 = mapper.requestToEntity(req1);
        assertNotNull(ent1);
        assertNull(ent1.getStrategy());
        assertNull(ent1.getSymbolData());

        StrategyRequest req2 = StrategyRequest.builder()
        ...
        StrategyRequest req3 = StrategyRequest.builder()
                .strategy(null)
                .symbolData(SymbolModel.builder().build())
                .build();
